...
def plot(epoches, performance, title, config):
    """
    ...
    plt.title(title+f' Seed={config.seed}')
    plt.xlabel('Epochs')
    plt.ylabel('Loss/Accuracy')
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'./imgs/{title}_Seed_{config.seed}.png')
    plt.show()
...
def run(config):
    ...
    plot(epoches, training_loss, 'Training_Loss', config)

    """plotting testing performance with the records"""
    plot(epoches, testing_accuracies, 'Testing_Accuracies', config)
    plot(epoches, testing_loss, 'Testing_Loss', config)
